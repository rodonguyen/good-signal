...
def export_chart_data(trades_df: pd.DataFrame, price_df: pd.DataFrame, equity_curve: pd.Series, output_file: Optional[str] = None) -> Dict:
    ...
    # Combine all data
    chart_data = {"candlestickData": candlestick_data, "equityData": equity_data, "trades": trades}
...
def export_chart_data_from_portfolio(
    portfolio_file: str, price_data_file: str, output_file: Optional[str] = None, symbol: Optional[str] = None
) -> Dict:
    ...
    # Get equity curve
    equity_curve = pd.Series(trades_df["equity"].values, index=trades_df["exit_time"]).sort_index()
