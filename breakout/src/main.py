...
def run_all_blocks(config: dict = None):
    ...
    engine = CryptoBreakoutEngine(
        atr_period=config["breakout"]["atr_period"],
        breakout_multiplier=config["breakout"]["breakout_multiplier"],
        stop_multiplier=config["breakout"]["stop_multiplier"],
        day_start_hour=config["breakout"]["day_start_hour"],
        fee_rate=config["breakout"]["fee_rate"],
        config_path=config["downloader"]["config_path"],
    )
    trades_df = engine.process_symbol(symbol=symbol, data_dir=config["breakout"]["data_dir"], output_dir=config["breakout"]["output_dir"])
...
Examples:
  ...
  python src/main.py --atr-period 20 --breakout-multiplier 0.5
    ...
    parser.add_argument("--data-dir", type=str, default="data/raw/crypto", help="Raw data directory")
    parser.add_argument("--trades-dir", type=str, default="data/trades", help="Trades output directory")
