...
class CryptoBreakoutEngine:
    ...
    def __init__(
        ...
        config_path: str = "config/crypto_symbols.yaml"
    ):
    ...
    def execute_trade(
        ...
        net_pnl = calculate_pnl_with_fees(
            entry_price, exit_price, size, direction, self.fee_rate
        )
    ...
    def process_symbol(
        ...
        for i, day_str in enumerate(unique_days):
            ...
            upper_level, lower_level = self.calculate_breakout_levels(
                prev_close, prev_atr, self.breakout_multiplier
            )
...
def main():
    ...
    parser.add_argument('--symbol', type=str, default=None,
                       help='Symbol to process (e.g., ETHUSDT). Defaults to config default.')
    parser.add_argument('--atr-period', type=int, default=14,
                       help='ATR period (default: 14)')
    parser.add_argument('--breakout-multiplier', type=float, default=0.33,
                       help='Breakout multiplier (default: 0.33)')
    parser.add_argument('--stop-multiplier', type=float, default=0.33,
                       help='Stop loss multiplier (default: 0.33)')
    parser.add_argument('--data-dir', type=str, default='data/raw/crypto',
                       help='Input data directory')
    parser.add_argument('--output-dir', type=str, default='data/trades',
                       help='Output trades directory')
    parser.add_argument('--config', type=str, default='config/crypto_symbols.yaml',
