...
class PortfolioBuilder:
    ...
    def calculate_position_size_fixed_dollar(
        self,
        trade: pd.Series,
    ...
    def calculate_position_size_equal_weight(
        ...
        trades_df['position_size'] = trades_df.groupby('symbol').apply(
            lambda group: capital_per_symbol / len(group) / group['entry_price']
    ...
    def apply_position_sizing(self, trades_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if method == 'risk_based':
            ...
            for idx, trade in trades_df.iterrows():
                size = self.calculate_position_size_risk_based(
                    trade, capital, risk_per_trade
                )
            ...
            trades_df['position_size'] = trades_df.apply(
                lambda row: self.calculate_position_size_fixed_dollar(row, amount_per_trade),
                axis=1
            )
...
def main():
    ...
    parser.add_argument('--config', type=str, default='config/portfolio_config.yaml',
